...
router.get(uspath, function(req,res){
	var results = [];
...
router.get(uspath+"/:id", function(req,res){
	var results = [];
    // Get a Postgres client from the connection pool
    pg.connect(connectionString, function(err, client, done) {
        // Handle connection errors
        ...
        var query = client.query("select s.song_id, s.song_title, s.song_genre, s.song_artist from user_song us, accounts a, songs s where us.user_id=a.user_id and s.song_id=us.song_id;");
...
router.put(pathname+"/:id", function(req, res) {
     var results = [];
